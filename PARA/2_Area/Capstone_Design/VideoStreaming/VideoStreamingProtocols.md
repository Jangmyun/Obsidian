...
Apple에서 개발한 프로토콜로, 인터넷에서 비디오를 스트리밍 하기 위해 사용

**ABR (Adaptive Bitrate)** Streaming 을 지원

### HLS 작동 방식

1. 동영상 인코딩 및 분할 (**segmentation**)
	- 원본 동영상을 다양한 해상도와 비트레이트로 인코딩
	- 인코딩한 동영상들을 2~10초 길이의 작은 세그먼트로 분할 (`.ts`파일)
2. **M3U8** 파일 생성
	- 세그먼트 파일들에 대한 메타데이터가 담긴 재생 목록 파일을 생성 (`.m3u8`파일)
	- `.m3u8`이 클라이언트에게 어떤 세그먼트 파일들을 어떤 순서로 재생해야 하는지 알려줌
3. 적응형 스트리밍
	- 클라이언트가 M3U8 파일을 먼저 다운로드하고 현재 자신의 네트워크 속도를 측정하여 가장 적합한 화질의 스트림을 선택하고 해당 세그먼트 파일을 순차적으로 요청하여 재생
...
![[Pasted image 20250812032558.png]]
...
파일 전체를 사용자에게 순차적으로 전송하면 전송된 일부를 재생하는 방식

다운로드 속도가 재생 속도보다 빨라야 버퍼링 없이 시청 가능

![[Pasted image 20250812121551.png]]
...
현재 사용자가 시청하고 있는 프레임만을 전송

이미 시청한 세그먼트는 버림

![[Pasted image 20250812121943.png]]


### Adaptive HTTP Streaming

전송하려는 파일 원본을 각 세그먼트 단위로 쪼개서 저장한 뒤 전송시에도 연속된 조각을 순차적으로 연결하여 전달하는 방식

![[Pasted image 20250812122029.png]]
...
1. 클라이언트 요청시 CDN은 사용자 위치를 감지하여 가까운 엣지 서버로 연결
2. 캐시된 콘텐츠가 있다면 엣지 서버가 직접 응답
3. 캐시에 없다면 엣지 서버가 원본 서버에서 데이터 받아와 제공
...
1. 영상 하나를 고화질 저화질 등 여러 비트레이트 혹은 해상도로 미리 인코딩

2. 인코딩 된 미디어 파일은 2~10초 단위로 쪼개진 세그먼트 형태로 저장

3. 클라이언트에서는 네트워크 속도, 하드웨어 사용량 등을 관찰하면서 상황에 맞는 최적의 화질을 선택해 재생

---

## MPEG-DASH

HTTP 기반 ABR 스트리밍 프로토콜

![[Pasted image 20250812123333.png]]

### 구조

- `.mpd` 파일
	- 기본 재생 정보
	- 전체 컨텐츠의 길이와 각 세그먼트 정보, 코덱 정보 등
	- **XML** 기반
	- MPD 한 파일 안에 여러 화질 정보 포함
- `.mp4`, `.m4s`
	- 메타데이터를 보관하고 이후 `.m4s` 파일로 실제 미디어 데이터만 보냄
	- 화질 변경 시 `init.mp4` 파일 새로 전송



HLS가 `.ts` 파일 안에 매번 메타데이터를 포함하는 반면
DASH는 `init.mp4` 파일 한번 전송 이후 미디어 데이터만 포함된 `.m4s`를 전송하여 메타데이터 분리
